...
|---|---|---|---|---|---|
| RF-001 | Capturar documento por cámara o selección de imagen — **una foto para `invoice_es`; una o más para `contract_es`** (2026-09-13, ver ADR-0003 sección "Sesión 2026-09-13": un contrato real suele tener campos obligatorios de RF-003 fuera de la portada) | `modules/camera`, `modules/documents` | `src/modules/camera/{errors,availability,resolution,use-camera-stream,CameraCapture}.{ts,tsx}`; multi-página: `src/modules/documents/upload-form.tsx` (`allowMultiplePages`), `src/modules/documents/actions.ts` (`createDocument`/`deleteDocument`), `supabase/migrations/20260913090000_create_document_pages.sql`, `src/app/(dashboard)/documents/[id]/page.tsx` (galería), `src/app/(dashboard)/documents/new/page.tsx`, `src/app/(dashboard)/contracts/new/page.tsx` | lógica pura de cámara: `tests/unit/modules/camera/*.test.ts` (18/18 verde); captura/preview/stream real: **sin test automatizado posible** (jsdom no implementa `getUserMedia`/`<video>`), pendiente de verificación manual — ver checklist en el cierre de Fase 3; RLS de `document_pages`: `tests/integration/document-pages-rls.test.ts` (10 casos, mismo patrón que `document_validations`) | IMPLEMENTED (no VERIFIED) — captura por cámara sigue pendiente de verificación manual (§11); `document_pages` implementado y con `tsc`/`eslint`/`build` limpios, pero su test de RLS **no corre en verde todavía**: la migración no se ha aplicado al proyecto Supabase real (falta `npx supabase db push --linked`, esta sesión no tiene el token de acceso) |
...
| RF-003 | Extracción de campos obligatorios — **dos perfiles** (ADR-0003, 2026-09-08): `invoice_es` (Proveedor, NIT, Fecha, IVA, Valor, Total; actualizado Fase 4e con datos reales de Mansor, facturación colombiana — reemplaza la definición de Fase 0 `proveedor/fecha/monto_total` + `numero_factura` deseado) y `contract_es` (Proveedor, NIT/documento, Fecha, Valor total, Vigencia, Tipo de contrato, Número de contrato) | `src/modules/ocr/classification/field-extraction.ts` (`invoice_es`), `contract-field-extraction.ts` (`contract_es`), primitivas compartidas en `field-extraction-helpers.ts`, dispatcher en `document-ocr-profile.ts` | `tests/unit/modules/ocr/classification/field-extraction.test.ts` (9/9), `contract-field-extraction.test.ts` (7/7), `document-ocr-profile.test.ts` (4/4) | `invoice_es` VERIFIED (heurística correcta sobre texto sintético; sin facturas reales de Mansor probadas todavía). `contract_es` IMPLEMENTED (no VERIFIED) — heurística razonable, sin dataset de contratos reales para probarla; usa Tesseract.js, no el pipeline propio (ver ADR-0003) |
